...
/**
 * Kanban board with drag-and-drop for tasks and project columns.
 *
 * Supports:
...
import { updateProjectOrder } from "@/actions/projects";
...
const SortableTaskCard = memo(function SortableTaskCard({ task, onSelect, onError }: { task: TaskItem; onSelect: (t: TaskItem) => void; onError?: (msg: string) => void }) {
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: task.id, data: { type: "task", task, columnId: getColumnId(task) } });
...
export function KanbanBoard({ tasks: propTasks, projects: propProjects }: KanbanBoardProps) {
  ...
  // Build columns: one per project (sorted) + unassigned
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    if (isColumnId(overIdStr)) return;
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    if (data?.type === "column") {
      ...
      try {
        await updateProjectOrder(activeId, newDisplayOrder);
      } catch {
        setLocalProjects(propProjects);
  ...
  return (
    ...
    >
      ...
      {/* Drag overlay — shows a floating copy of the card or column being dragged */}
      <DragOverlay>
        {activeTask ? (
