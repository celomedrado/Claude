/**
 * Paste & Extract page — adapted for Tauri IPC.
 *
 * Changes: uses api.extractTasks instead of fetch, uses react-router-dom navigate.
 */

import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { listProjects, extractTasks, bulkCreateTasks } from "@/lib/api";
import { emitDataChanged } from "@/components/app-shell";
import { Button } from "@/components/ui/button";
import { Sparkles, Check, X, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import type { ExtractedTask, Project, CreateTaskInput } from "@/lib/types";
      ...
      <div className="space-y-6">
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 flex justify-end">
            <Button onClick={handleExtract} disabled={loading || !text.trim()}>
              {loading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Extracting...</>) : (<><Sparkles className="mr-2 h-4 w-4" />Extract Tasks</>)}
            </Button>
          </div>
        </div>
        ...
        {extracted.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-700">Extracted {extracted.length} task{extracted.length !== 1 ? "s" : ""} — review and save</h3>
              <span className="text-xs text-gray-400">{selected.size} selected</span>
            </div>
            <div className="space-y-2">
              {extracted.map((task, i) => (
                <div key={i} className={cn("rounded-lg border bg-white p-4 shadow-sm transition-colors", selected.has(i) ? "border-indigo-300 bg-indigo-50/30" : "border-gray-200 opacity-50")}>
                  <div className="flex items-start gap-3">
                    <button onClick={() => toggleTask(i)} className={cn("mt-0.5 shrink-0 rounded-md border p-1", selected.has(i) ? "border-indigo-500 bg-indigo-500 text-white" : "border-gray-300 text-transparent")}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <span className={cn("rounded-full px-2 py-0.5 text-xs font-medium", PRIORITY_BADGES[task.priority])}>{task.priority}</span>
                        ...
                        >
                          <option value="">{task.suggestedProject ? `Suggested: ${task.suggestedProject}` : "No project"}</option>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="ghost" onClick={() => { setExtracted([]); setSelected(new Set()); setProjectMap(new Map()); }}>
                <X className="mr-1 h-4 w-4" />Discard
              </Button>
              <Button onClick={handleSave} disabled={saving || selected.size === 0}>
                {saving ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</>) : (<><Check className="mr-2 h-4 w-4" />Save {selected.size} Task{selected.size !== 1 ? "s" : ""}</>)}
              </Button>
            </div>
          </div>
        )}
        ...
        {extracted.length === 0 && !loading && text.trim() === "" && (
          <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400">Paste text above and click Extract to get started</p>
          </div>
        )}
      </div>
    </div>
